...
const LOCAL_DRAWING_ENTRY_PATH = '/api/local/drawing-entry';
const LOCAL_MERCHANT_BUILD = '408-LOCAL-1.7';
...
const MAX_LOCAL_DRAWING_BODY_BYTES = 16 * 1024;
const MAX_LOCAL_EVENT_BODY_BYTES = 6 * 1024;
const LOCAL_MERCHANT_CATEGORIES = new Set(['eat-drink','home','auto','health-fitness','storage-property','professional','other']);
const LOCAL_PARTNERSHIP_INTERESTS = new Set(['local-perk','visibility','both','not-sure']);
...
function coverageFitLeadPayload(fields) {
  const allowed = [
    ...
    ['closing_date',40],['property_zip',10],['property_type',40],['current_carrier',60],['renewal_timing',40],['auto_vehicle_count',20],['fiv_signal_version',80],
...
function normalizeCallbackSchedule(value) {
  ...
  if (!validPhone(phone) || !phone || !new Set(['home','condo','auto','life','business','general']).has(productType)) return null;
...
async function handleLocalMerchantApplication(request, env) {
  ...
  const application = {
    ...
    partnership_interest: text(fields.get('partnership_interest'), 30),
    proposed_perk: text(fields.get('proposed_perk'), 700),
    proposed_placement: text(fields.get('proposed_placement'), 700),
    ...
    campaign: text(fields.get('campaign'), 120) || '408 Local Merchant Pilot',
  ...
  if (!application.business_name || !application.business_location || !application.contact_name || !application.email || !application.phone || !application.partnership_interest) return fail(400);
  if (!LOCAL_MERCHANT_CATEGORIES.has(application.category)) return fail(400);
  if (!LOCAL_PARTNERSHIP_INTERESTS.has(application.partnership_interest)) return fail(400);
  if ((application.partnership_interest === 'local-perk' || application.partnership_interest === 'both') && !application.proposed_perk) return fail(400);
  if ((application.partnership_interest === 'visibility' || application.partnership_interest === 'both') && !application.proposed_placement) return fail(400);
  ...
  const outbound = new URLSearchParams();
  outbound.set('_subject', '408 Local Merchant Application');
...
async function handleLocalDrawingEntry(request, env) {
  if (request.method !== 'POST') return fail(405);
  const contentType = String(request.headers.get('content-type') || '').toLowerCase();
  if (!contentType.includes('application/x-www-form-urlencoded') && !contentType.includes('multipart/form-data')) return fail(415);
  const lengthHeader = Number(request.headers.get('content-length') || 0);
  if (Number.isFinite(lengthHeader) && lengthHeader > MAX_LOCAL_DRAWING_BODY_BYTES) return fail(413);

  let fields;
  try { fields = await request.formData(); } catch (_) { return fail(400); }
  if (text(fields.get('_gotcha'), 200)) return jsonResponse(200, { ok:true });

  const entry = {
    first_name: text(fields.get('first_name'), 80),
    last_name: text(fields.get('last_name'), 80),
    mobile: text(fields.get('mobile'), 30),
    email: text(fields.get('email'), 160),
    company: text(fields.get('company'), 120),
    entry_ack: text(fields.get('entry_ack'), 10),
    insurance_opt_in: text(fields.get('insurance_opt_in'), 10) === 'yes' ? 'yes' : 'no',
    source: text(fields.get('source'), 120) || '408farmers.com/local/stevies-bar-grill/drawing',
    campaign: text(fields.get('campaign'), 120) || 'Stevies Monthly Drawing',
    landing_page: text(fields.get('landing_page'), 500),
    submitted_at: text(fields.get('submitted_at'), 60),
    utm_source: text(fields.get('utm_source'), 120),
    utm_medium: text(fields.get('utm_medium'), 120),
  ...
  if (!entry.first_name || !entry.last_name || (!entry.mobile && !entry.email) || entry.entry_ack !== 'yes') return fail(400);
  if (entry.email && !validEmail(entry.email)) return fail(400);
  if (entry.mobile && !validPhone(entry.mobile)) return fail(400);

  const outbound = new URLSearchParams();
  outbound.set('_subject', entry.insurance_opt_in === 'yes'
    ? "Stevie's Drawing Entry - OPTIONAL INSURANCE REVIEW REQUESTED"
    : "Stevie's $50 Gift Card Drawing Entry");
  outbound.set('form_type', 'local_drawing_entry');
  for (const [key, value] of Object.entries(entry)) outbound.set(key, value);

  const endpoint = String((env && env.LOCAL_DRAWING_FORMSPREE_ENDPOINT) || (env && env.FORMSPREE_ENDPOINT) || DEFAULT_FORMSPREE_ENDPOINT).trim();
  if (!/^https:\/\/formspree\.io\/f\/[A-Za-z0-9_-]+$/.test(endpoint)) return fail(503);
  let upstream;
  try {
    upstream = await fetch(endpoint, { method:'POST', body:outbound, headers:{'Accept':'application/json','Content-Type':'application/x-www-form-urlencoded;charset=UTF-8','Referer':'https://408farmers.com/local/stevies-bar-grill/drawing/'}, redirect:'manual' });
  } catch (_) { return fail(502); }
  if (!upstream.ok) return jsonResponse(upstream.status >= 400 && upstream.status < 600 ? upstream.status : 502, { ok:false, error:'drawing_entry_delivery_failed' });
  return jsonResponse(200, { ok:true, insurance_opt_in:entry.insurance_opt_in });
}
...
function pageAssetRoute(pathname) {
  ...
  // Short physical-world campaign routes. Add approved partners here without
  // changing public landing-page structure or attribution contracts.
  const goCampaigns = Object.freeze({
    stevies: '/local/stevies-bar-grill/drawing/?utm_source=stevies&utm_medium=counter_qr&utm_campaign=monthly_stevies_drawing&utm_content=5x7_sign'
  });
  const goMatch = path.match(/^\/go\/([a-z0-9]+(?:-[a-z0-9]+)*)\/?$/);
  if (goMatch && goCampaigns[goMatch[1]]) {
    return { redirect: goCampaigns[goMatch[1]], status: 302 };
  }
  ...
  const canonicalDirectories = new Set(['/home','/condo','/contact','/buyer','/life','/life-ops','/neighbor','/snapshot','/auto-bundle','/healthcare','/teachers','/tech','/engineers','/local']);
  ...
  if (path === '/local/stevies-bar-grill/drawing') return { redirect: '/local/stevies-bar-grill/drawing/', status: 308 };
...
// Durable synthetic records only. There is intentionally no outbound adapter.
async function persistSignalLifeStaging(env, body, signals, origin) {
  const db=env && env.SIGNAL_HANDOFF_PREVIEW_DB;
  if(!db || typeof db.prepare!=='function')return {error:'staging_binding_missing',statusCode:503};
  try {
    const marker=await db.prepare('SELECT scope, synthetic_only, delivery_enabled FROM signal_preview_environment WHERE singleton = 1').first();
    if(!marker || marker.scope!=='408-signal-handoff-preview-v1' || marker.synthetic_only!==1 || marker.delivery_enabled!==0)return {error:'staging_environment_unverified',statusCode:503};
    const h=body.handoff,now=Date.now();
    // Expiration does not renew on retry. No identity/IP/user-agent is stored.
    const canonical=JSON.stringify(Object.fromEntries(Object.keys(signals).sort().map(k=>[k,signals[k]])));
    const answers=JSON.stringify(h.answers.map(a=>({questionId:a.questionId,questionVersion:a.questionVersion,optionCode:a.optionCode,signals:a.signals,answeredAt:a.answeredAt})));
    const permission={version:body.permission.version,channel:body.permission.channel,checked:true,scope:'simulation_only',contactPermissionGranted:false,receivedAt:new Date(now).toISOString()};
    const semantic=JSON.stringify({schema:'SIGNAL-LIFE-1.2',session:h.signalSessionId,mode:body.mode,time:body.timePreference,phone:body.phone,canonical,answers,permissionVersion:permission.version});
    const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(semantic));
    const id='staging-'+Array.from(new Uint8Array(digest)).map(v=>v.toString(16).padStart(2,'0')).join('');
    await db.prepare('DELETE FROM signal_handoff_staging WHERE expires_at <= ?').bind(now).run();
    // One row holds both evidence and simulated permission. PK is the concurrency boundary.
    await db.prepare(`INSERT INTO signal_handoff_staging
      (request_id,schema_version,signal_session_id,contact_mode,synthetic_phone,time_preference,canonical_signals_json,answers_json,permission_evidence_json,source_origin,status,delivery_state,created_at,expires_at)
      SELECT ?,?,?,?,?,?,?,?,?,?,'staging_only','disabled',?,?
      WHERE (SELECT COUNT(*) FROM signal_handoff_staging) < 5000
      ON CONFLICT(request_id) DO NOTHING`).bind(id,'SIGNAL-LIFE-1.2',h.signalSessionId,body.mode,body.phone,body.timePreference,canonical,answers,JSON.stringify(permission),origin,now,now+7*86400000).run();
    const row=await db.prepare('SELECT request_id, created_at, expires_at FROM signal_handoff_staging WHERE request_id = ?').bind(id).first();
    if(!row)return {error:'staging_capacity_reached',statusCode:503};
    return {requestId:row.request_id,createdAt:new Date(row.created_at).toISOString(),expiresAt:new Date(row.expires_at).toISOString()};
  }catch{return {error:'staging_storage_unavailable',statusCode:503};}
}

async function handleSignalLifeHandoffPreview(request, stagingEnv = null) {
  const durable = new URL(request.url).pathname === '/api/signal/life-handoff-staging';
  ...
  if (!strict(body,['schemaVersion','mode','phone','timePreference','permission','handoff']) || body.schemaVersion!==(durable?'SIGNAL-LIFE-1.2':'SIGNAL-LIFE-1.1') || !modes.includes(body.mode) || body.phone!=='+12025550123') return reply(400,{error:'synthetic_contact_required'});
  ...
  if(durable){
    const saved=await persistSignalLifeStaging(stagingEnv,body,signals,origin);
    if(saved.error)return reply(saved.statusCode,{error:saved.error});
    return reply(200,{schemaVersion:'SIGNAL-LIFE-1.2',status:'staging_recorded',receipt:saved.requestId,createdAt:saved.createdAt,expiresAt:saved.expiresAt,mode:body.mode,timePreference:slot,canonicalSignals:signals,answerCount:h.answers.length,guardrails:{syntheticContactOnly:true,stagingRecordPersisted:true,simulatedPermissionEvidenceStored:true,leadCreated:false,opportunityCreated:false,callbackCreated:false,consultationCreated:false,contactPermissionGranted:false,messageSent:false,deliveryEnabled:false}});
  }
...
export default {
  async fetch(request, env, executionContext) {
    ...
    if(['GET','HEAD'].includes(request.method)){
      ...
      if(['/teachers/legacy.html','/teachers/legacy'].includes(url.pathname))return env.ASSETS.fetch(assetRequestFor(request,'/teachers/legacy'));
      if(['/healthcare/legacy.html','/healthcare/legacy'].includes(url.pathname))return env.ASSETS.fetch(assetRequestFor(request,'/healthcare/legacy'));
      if(['/engineers/legacy.html','/engineers/legacy'].includes(url.pathname))return env.ASSETS.fetch(assetRequestFor(request,'/engineers/legacy'));
    }
    ...
    if (url.pathname === LOCAL_DRAWING_ENTRY_PATH) return handleLocalDrawingEntry(request, env);
