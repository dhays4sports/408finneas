/* 408-LOCAL-1.7 — Merchant + Visibility Partner Join Flow */
(function (root, factory) {
  ...
  const BUILD = '408-LOCAL-1.7';
  const UTM_FIELDS = Object.freeze(['utm_source','utm_medium','utm_campaign','utm_content','utm_term']);
  const REQUIRED_FIELDS = Object.freeze(['business_name','category','business_location','contact_name','email','phone','partnership_interest','authorized_ack','separation_ack']);
  ...
  function validateFormData(formData) {
    ...
    if (!['eat-drink','home','auto','health-fitness','storage-property','professional','other'].includes(category)) return { ok: false, error: 'Choose a business category.' };
    const interest = text(formData.get('partnership_interest'), 30);
    if (!['local-perk','visibility','both','not-sure'].includes(interest)) return { ok: false, error: 'Choose a partnership interest.' };
    const perk = text(formData.get('proposed_perk'), 700);
    const placement = text(formData.get('proposed_placement'), 700);
    if ((interest === 'local-perk' || interest === 'both') && !perk) return { ok: false, error: 'Tell us what kind of Local offer or gift-card promotion you would consider.' };
    if ((interest === 'visibility' || interest === 'both') && !placement) return { ok: false, error: 'Tell us what kind of visibility area you may be open to discussing.' };
  ...
  function setBusy(form, busy) {
    ...
    button.textContent = busy ? 'Sending request…' : 'Send partnership request';
  }
