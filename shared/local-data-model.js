  ...
  function validateCatalog(catalog) {
    ...
    catalog.merchants.forEach((merchant, index) => {
      ...
      ['name','description_short','description_long'].forEach((key) => {
        if (hasForbiddenEndorsementLanguage(merchant[key])) add(`${p}.${key}`, 'must not imply Farmers Insurance endorsement');
      });
    });
    ...
    catalog.perks.forEach((perk, index) => {
      ...
      ['headline','summary','terms','independent_offer_text'].forEach((key) => {
        if (hasForbiddenEndorsementLanguage(perk[key])) add(`${p}.${key}`, 'must not imply Farmers Insurance endorsement');
      });
    });
