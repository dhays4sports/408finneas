...
form.addEventListener('submit',async function(e){
 ...
 try{
  var response=await fetch('/api/signal/life-handoff-preview',{method:'POST',credentials:'omit',headers:{'Content-Type':'application/json'},signal:controller.signal,body:JSON.stringify({schemaVersion:'SIGNAL-LIFE-1.1',mode:mode,phone:'+12025550123',timePreference:slot,permission:{version:'preview-permission-v1',channel:mode==='text'?'sms':'phone',checked:true},handoff:handoff})});
  var result=await response.json();
  if(!response.ok||result.status!=='preview_validated')throw new Error('Preview unavailable');
  status.textContent='Preview handoff verified. '+result.answerCount+' answers carried forward.\nNo call, text, appointment, lead or contact-permission record was created.\nReceipt: '+result.receipt;
