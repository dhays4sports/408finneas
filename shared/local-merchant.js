(function (root, factory) {
  ...
  function renderMerchantDetail(viewModel) {
    ...
    const relationshipText = viewModel.program && viewModel.program.merchant_relationship_text
      ...
      : 'Participation in 408 Local does not imply endorsement, certification, or recommendation by Farmers Insurance, Dylan Haysbert, or Virginia Tam Insurance Agency, Inc.';
    return [
      ...
      '<section class="local-detail-boundary" aria-label="408 Local program boundary">',
  ...
  function renderUnavailable(reason) {
    ...
    return [
      ...
      '<p class="local-kicker">408 LOCAL</p>',
  ...
  function updateMetadata(doc, viewModel) {
    ...
    const m = viewModel.merchant;
    const title = `${m.name} | 408 Local`;
    const description = viewModel.perk
      ? `${viewModel.perk.headline} — a current 408 Local offer from ${m.name}. No insurance purchase or quote required.`
      : `${m.name} on 408 Local. No insurance purchase or quote required.`;
    doc.title = title;
    ...
    return LocalDataModel.loadCatalog('/local/data/catalog.json')
      ...
      .catch(() => {
        ...
        root.innerHTML = [
          ...
          '<p>Please return to the directory and try again later. Insurance reviews on Dylan’s insurance site are unaffected.</p>',
